...
class CarNeuralNetwork(nn.Module):
    ...
    def __init__(self, input_size=5, hidden_size=8, output_size=2):
        """
        Args:
            input_size: Number of sensor inputs (default 5)
            hidden_size: Number of neurons in hidden layer
            output_size: Number of outputs (acceleration, steering)
        """
        super(CarNeuralNetwork, self).__init__()
    ...
    def predict(self, sensor_data):
        ...
        Returns:
            action: [acceleration, steering] in range [-1, 1]
        """
        with torch.no_grad():
